    ...
    <Shell userName={name || getUserName()} active={active} onNavigate={onNavigate} onOpenSettings={onOpenSettings} onBackHome={onBackHome} left={left}>
      <div className="flex-1 overflow-y-auto p-6 lg:p-10">
        <div className="w-full max-w-3xl mx-auto animate-[fadeUp_.4s_ease]">
          ...
          <div className="lg:hidden flex items-center gap-2 overflow-x-auto pb-4 -mx-1 px-1 max-w-full [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {SECTIONS.map((s) => (
              <button
                key={s.id}
                onClick={() => setSection(s.id)}
                className={`flex items-center gap-2 px-3.5 py-2 rounded-xl border text-xs font-semibold whitespace-nowrap transition ${
                  section === s.id
                    ? 'bg-gradient-to-r from-[#4169E1]/70 to-[#7C3AED]/70 border-white/20 text-white shadow-lg shadow-[#4169E1]/25'
                    : 'bg-white/[0.06] border-white/10 text-white/60 hover:bg-white/10 hover:text-white'
                }`}
              >
            ...
            <>
              ...
              <div className="bg-white/[0.06] backdrop-blur-2xl border border-white/10 rounded-2xl p-6">
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  ...
                  <button
                    onClick={saveName}
                    className={`bg-gradient-to-r px-5 py-2 rounded-lg text-sm font-semibold text-white transition shadow-lg shadow-[#4169E1]/30 w-full sm:w-auto ${
